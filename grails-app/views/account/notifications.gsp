    ...
    <g:each in="${notifications}" var="noty" status="i">
        ...
        <div class="form-group">
            <g:if test="${noty?.type?.type.equals("purshasequotation")}">
                ...
                <h3 class="notificationline">${conprise.PurshaseForm?.get(noty?.formId)?.user?.name} ${noty?.type?.text}</h3>
            ...
            <g:if test="${noty?.type?.type.equals("purshaceorder")}">
                <h3 class="notificationline"
                    class="notificationline">${noty?.type?.text} from ${conprise.SalesForm?.get(noty?.formId)?.user?.name}</h3>
            ...
            <g:if test="${noty?.type?.type.equals("salesorder")}">
                <h3 class="notificationline">${noty?.type?.text} from ${conprise.SalesForm?.get(noty?.formId)?.purshaseForm?.user?.name}</h3>
                <g:link action="watchSalesOrder" id="${noty?.formId}"   params="['notification':noty?.id]"><button
                        class="pull-left btn btn-primary">Open</button></g:link>
            </g:if>
            <g:if test="${noty?.type?.type.equals("tracking")}">
                <h3 class="notificationline">${noty?.type?.text} from ${conprise.SalesForm?.get(noty?.formId)?.user?.name}</h3>
            ...
            <g:if test="${noty?.type?.type.equals("delivery")}">
                <h3 class="notificationline">${conprise.SalesForm?.get(noty?.formId)?.purshaseForm?.user?.name} ${noty?.type?.text} .</h3>
            ...
            <g:if test="${noty?.type?.type.equals("invoicesubmitted")}">
                <h3 class="notificationline">${noty?.type?.text} by ${conprise.SalesForm?.get(noty?.formId)?.purshaseForm?.user?.name} .</h3>
            ...
            <g:if test="${noty?.type?.type.equals("friendrequestaccept")}">
                <h3 class="notificationline">${conprise.User.get(noty?.formId).name} ${noty?.type?.text}</h3>
